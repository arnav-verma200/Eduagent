...
EduAgent is a full-stack academic assessment platform designed to move beyond traditional grade-based testing. By leveraging a multi-agent cognitive architecture powered by **Google Gemini, Gemma 4, and Antigravity** models, EduAgent evaluates not just *what* answers a student gets wrong, but *why* they got them wrong. 

The platform runs a multi-agent evaluation pipeline that diagnoses cognitive profiles, performs adversarial follow-up probing, detects behavioral anomalies (without making definitive accusations), and compiles class-wide learning analytics. It features a robust model fallback chain that prioritizes performance and reliability across multiple model classes, starting with Gemma 4 and Antigravity, and scaling down to Gemini models.
...
The database uses a PostgreSQL instance hosted on Supabase with the following tables:
...
### 2. `results`
Stores full multi-agent evaluations, integrity flags, generated probes, depth diagnostics, and Socratic debate confirmations.
```sql
create table results (
  ...
  confirmed_diagnosis jsonb, -- Post-Socratic debate confirmed diagnoses per question
  created_at timestamp default now(),
  unique(exam_id, student_id) -- Prevents duplicate submissions
);
```

### 3. `debates`
Stores Socratic debate sessions between students and the AI examiner.
```sql
...
- **Backend**: Python 3.11+, FastAPI, Uvicorn, Google GenAI SDK (supporting Gemma 4, Antigravity, and Gemini models via an automated fallback chain), PyMuPDF (PDF text parsing), python-dotenv, Supabase Python Client.
